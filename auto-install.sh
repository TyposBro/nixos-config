...
if [ -d "setup" ]; then
  echo "setup exists, backing up to .config/setup-backups folder."
  if [ -d ".config/setup-backups" ]; then
    echo "Moving current version of setup to backups folder."
    mv "$HOME"/setup .config/setup-backups/"$backupname"
    sleep 1
  else
    echo "Creating the backups folder & moving setup to it."
    mkdir -p .config/setup-backups
    mv "$HOME"/setup .config/setup-backups/"$backupname"
    sleep 1
  fi
else
  echo "Thank you for choosing setup."
  echo "I hope you find your time here enjoyable!"
fi

echo "-----"

echo "Cloning & Entering setup Repository"
git clone --single-branch https://github.com/TyposBro/setup.git
cd setup || exit
